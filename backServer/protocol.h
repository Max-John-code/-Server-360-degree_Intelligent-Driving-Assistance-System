...
//15 - 图片信息保存数据库结构体 16 - 图片查询协议体 17 - 图片信息返回包 
// 18 - 图片信息保存返回 19 - 图片查询返回
...
typedef struct imageFinish//图片信息上传完成
{
	char img_name[150];//图片名称
	char account[10];//用户账号
}IMAGEOKT;
...
typedef struct imageback//图片拼合完成返回包
{
	int flag;//0 拼合完成  1 数据包丢失
	char img_path[150];//图片路径
...
typedef struct getPhotosInfo
{
	...
	int page;                    //需要的图片信息列表的页数 （按每页9个数据计算）//返回总页数，用于边界判断
