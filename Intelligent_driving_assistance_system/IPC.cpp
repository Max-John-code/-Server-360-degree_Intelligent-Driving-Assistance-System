...
IPC::IPC()
{
	...
	{
		sem_setVal(this->semID, i, 1);						//信号量初始化
	}
	cout << "信号量创建完成" << endl;
...
void IPC::setShm(char resbuf[])
{
	...
	cout << "sem_id =" << shmID << endl;
	cout << "sem_index =" << sndIndex << endl;
	//对应信号量加锁
	sem_p(this->shmID,this->sndIndex);
	cout << "sem_id =" << shmID << endl;
	cout << "sem_index =" << sndIndex << endl;
	//数据写入共享内存
	...
	*/
	...
	//对应信号量解锁
	sem_v(this->shmID,this->sndIndex);
...
void IPC::getShm()
{
	...
	{
		sem_p(this->shmID,this->rsvIndex);		//对应信号量加锁
		...
		//cout << "测试读取返回包体" << endl;
		//cout << "body.flag = " << body.result << endl;
		cout << "-----------------------------------------------" << endl; 
		...
		sem_v(this->shmID,this->rsvIndex);		//对应信号量解锁
...
void IPC::printfLog(int allNum)
{
	...
	//cout << "    上传特征图片：" << workNum[10] << "  查询图片数据：" << workNum[11] << endl;
	cout << "========================================================================" << endl;
}
